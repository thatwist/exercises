...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) {
      ...
      val (x, xs) = (initial.head, initial.tail)
      if (done(x._1)) {
        Stream((x._1, x._2)) 
      } else {
        val expl  = explored.union(Set(x._1))
        val newno = newNeighborsOnly(neighborsWithHistory(x._1, x._2), expl)
        Stream((x._1, x._2))++from(xs++newno, expl)
      }
    }
  }
