...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  def union(that: TweetSet): TweetSet = unionAcc(that.unionAcc(new Empty()))
...
class Empty extends TweetSet {
  ...
  override def mostRetweetedMax(max: Tweet): Tweet = max 
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def unionAcc(acc: TweetSet): TweetSet = 
    left.unionAcc(right.unionAcc(acc.incl(elem)))

  override def mostRetweeted: Tweet = 
    mostRetweetedMax(elem)

  override def mostRetweetedMax(max: Tweet): Tweet = 
    left.mostRetweetedMax(right.mostRetweetedMax(
...
object GoogleVsApple {
  ...
  def filter(keywords: List[String]): TweetSet = 
    TweetReader.allTweets.filter(x => keywords.exists(y => x.text.contains(y)))
